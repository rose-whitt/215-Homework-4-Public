...
class PyFloatNodeTest extends APyNodeTest {
    ...
    /**
     * A PyFloatNode whose domains include a single integer (using different integers for
     * the exhaustive and random domains).
     */
    static PyFloatNode singleInt;
    ...
    static void setUp() {
        ...
        List<Number> exDomain = Collections.singletonList(17.654321);
        List<Number> ranDomain = Collections.singletonList(203.123456);
        ...
        singleInt = new PyFloatNode();
        exDomain = Collections.singletonList(3);
        ranDomain = Collections.singletonList(1);
        singleInt.setExDomain(exDomain);
        singleInt.setRanDomain(ranDomain);
    ...
    void testGetExDomain() {
        List<Number> exDomain = singlePos.getExDomain();
    ...
    void testGetRanDomain() {
        List<Number> ranDomain = singlePos.getRanDomain();
    ...
    /**
     * Tests genExVals() in the case that the domain consists of a single integer.
     */
    @Test
    @Tag("0.25")
    @Order(7)
    void testGenExValsInt() {
        Set<PyFloatObj> actual = singleInt.genExVals();
        Set<PyFloatObj> expected = Set.of(new PyFloatObj(3.0));
        assertEquals(expected, actual);
    }
    ...
    /**
     * Tests genRandVal() in the case that the domain consists of a single integer.
     */
    @Test
    @Tag("0.25")
    @Order(11)
    void testGenRandValInt() {
        Map<PyFloatObj, Double> actual = buildDistribution(singleInt, 100);
        Map<PyFloatObj, Double> expected = Map.of(new PyFloatObj(1.0), 1.0);
        assertTrue(compareDistribution(expected, actual, 0.0));
    }
